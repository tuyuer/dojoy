        ...
        protected virtual void FixAimPoseFK()
        {
            if (RC.Aim)
            {
                if (RC.Aim && RC.WeaponAction != WA.ReloadRight && RC.WeaponAction != WA.ReloadLeft)
                {
                    ...
                    Vector3 ShoulderRotationAxis = RC.Aimer.ForcedTarget ? Vector3.Cross(UpVector, RC.AimDirection).normalized : RC.Aimer.MainCamera.transform.right;
