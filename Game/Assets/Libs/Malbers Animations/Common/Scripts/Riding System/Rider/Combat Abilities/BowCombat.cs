    ...
    public class BowCombat : RiderCombatAbility
    {
        ...
        public override void OnWeaponAim(bool aim)
        {
            if (!aim && isHolding)
            {
                ...
                Bow.BendBow(0);                            //Reset the bending of the bow
            }
        }
        ...
        protected virtual void FixAimPoseBow()
        {
            if (RC.Aim)
            {
                ...
                Vector3 ShoulderRotationAxis = RC.Aimer.ForcedTarget ? Vector3.Cross(UpVector, AimDirection).normalized : RC.Aimer.MainCamera.transform.right;
        ...
        {
            return (RC.ActiveWeapon as IBow).KNot;
        }
    }
}