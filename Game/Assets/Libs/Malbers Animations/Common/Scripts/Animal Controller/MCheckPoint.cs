...
namespace MalbersAnimations.Controller
{
    public class MCheckPoint : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            {
                Debug.LogError(name + " has being destroyed since there's no Respawner");
                Destroy(gameObject); //Destroy the CheckPoint if is there no Respawner
            }

            if (MAnimal.MainAnimal == null)
            {
                Debug.LogError(name + " has being destroyed since there's no Main Animal Player, Set on your Main Character: Main Player = true");
                Destroy(gameObject); //Destroy the CheckPoint if is there no Respawner
            }
