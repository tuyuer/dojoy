   ...
   // [RequireComponent(typeof(Rigidbody))]
    public partial class MAnimal : MonoBehaviour, IAnimatorListener, ICharacterMove, IGravity, IMDamage, IMHitLayer// ,IInteractable
    {
        ...
        void CreateListeners()
        {
            ...
            //MEvent SetSpeed = MalbersTools.GetInstance<MEvent>("Set Speeds");
            //if (listener.Events.Find(item => item.Event == SetSpeed) == null)
            //{
            //    var item = new MEventItemListener()
            //    {
            //        Event = SetSpeed,
            ...
            //    UnityEditor.Events.UnityEventTools.AddPersistentListener(item.Response, SpeedDown);
            //    UnityEditor.Events.UnityEventTools.AddPersistentListener(item.ResponseBool, SpeedUp  );

            //    listener.Events.Add(item);

            //    Debug.Log("<B>Set Speeds</B> Added to the Event Listeners");
            //}
        }
        ...
        void SetModesListeners(MEventListener listener ,string EventName, string ModeName)
        {
            ...
            if (listener.Events.Find(item => item.Event == e) == null)
            {
                ...
                {
                    Event = e,
                    useVoid = false, useInt = true, useBool = true,
                };
